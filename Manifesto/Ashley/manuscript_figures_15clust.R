...
permanova1<-adonis(cover.Bio2~data2$treatment*as.factor(data2$year), strata=data2$shelterBlock, perm=1000, method="bray")
permanova1
...
data3$TB<-paste(data3$treatment,data3$shelterBlock,sep=".")
data3[is.na(data3)] <- 0


cover.Bio3<- data3 %>% dplyr::select(-c(1:6), -ID,-TB)
...
##flexible beta method
spp.clust.beta15<-agnes(spp.bcd3, method="flexible", par.method=0.625)#par.method as 0.625 is equivalent to beta -0.25
spp.clust.beta15<-as.hclust(spp.clust.beta15)#match format of hclust
plot(spp.clust.beta15, cex=0.8) #2 clusters

#loop calculates a df with cluster membership from k=2 to k=10
clustmem15<-c()
for (i in 2:10){
  temp.clust<-cutree(spp.clust.beta15,k=i)
  clustmem15<-cbind(clustmem15,temp.clust)
}
clustmem15<-data.frame(clustmem15)
colnames(clustmem15)<-paste("clust",c(2:10),sep="")
clustmem15

#cluster membership can be treated like a categorical variable
data3$clust15<-clustmem15$clust2
data3b<-data3 %>% dplyr::select(treatment, shelterBlock, clust15)
data4<-merge(May_all_XC, data3b)
...
points(spp.mds2, display="sites", cex=0.8, pch=16, col=data4$clust15)
...
points(spscoresall_2$NMDS1,spscoresall_2$NMDS2,col=data4$clust15,pch=shapes, cex=1.2)#Plot the ordination points 
...
Fig1c<-ggplot(spscoresall_2, aes(x=NMDS1, y=NMDS2))+
  xlim(-1,1)+
  geom_path(data=spscoresall_2, arrow=arrow(), mapping=aes(x=NMDS1, y=NMDS2, group=TB))+
  geom_point(cex=5, aes(shape=as.factor(data4$treatment2), fill=as.factor(data4$clust15)))+
...
spscoresall_2$clust15<-data4$clust15
clust1 <- spscoresall_2[spscoresall_2$clust15 == "1", ][chull(spscoresall_2[spscoresall_2$clust15 == 
                                                                   "1", c("NMDS1", "NMDS2")]), ]  # hull values for cluster 1
clust2 <- spscoresall_2[spscoresall_2$clust15 == "2", ][chull(spscoresall_2[spscoresall_2$clust15 == 
...
Fig1a<-ggplot(spscoresall_2, aes(x=NMDS1, y=NMDS2))+
  xlim(-1,1.3)+
  geom_polygon(data=hull.data,aes(x=NMDS1,y=NMDS2,fill=as.factor(clust15),group=as.factor(clust15)),alpha=0.6) + # add the convex hulls
...
geom_path(data=spscoresall_2, arrow=arrow(), mapping=aes(x=NMDS1, y=NMDS2, group=TB))+
  geom_point(cex=4, aes(shape=as.factor(data4$treatment2), fill=data4$treatment2))+
...
clust_xc<-data4 %>% filter(year==2015) #%>% dplyr::select(treatment, shelterBlock, clust2)
veclength<-veclength %>% arrange(treatment,shelterBlock)
veclength2<-merge(clust_xc,veclength) %>%filter(clust15==1|clust15==2)

v2<-lme(veclength ~ clust15, random=~1|treatment/shelterBlock, veclength2, na.action=na.exclude)
summary(v2)
anova(v2)
r.squaredGLMM(v2) 
qqnorm(residuals(v2))
...
Fig1d<- ggplot(data=veclength2, aes(x=as.factor(clust15), y=veclength, fill=as.factor(clust15), alpha=0.8))+
  ylim(0,1.5)+
  geom_boxplot()+
...
xc_isa = multipatt(cover.relrow2, data4$clust15, control=how(nperm=999))
summary(xc_isa)

#indicator species by treatment
trt_isa = multipatt(cover.relrow2, data4$treatment, control=how(nperm=999))
summary(trt_isa)

#test for cluster differences in community composition
permanova2<-adonis(cover.Bio2~data4$clust15*as.factor(data2$year), strata=data2$block, perm=1000, method="bray")
permanova2

##within cluster 1, do compensatory dynamics occur?
ggplot(data4, aes(y=propForb, x=treatment, fill=as.factor(data4$clust15)))+
  geom_boxplot()

ggplot(data4, aes(y=RaoQ, x=as.factor(clust15)))+
  geom_boxplot()

mR<-lme(RaoQ ~treatment*as.factor(clust15), random=~1|year/shelterBlock, data4, na.action=na.exclude)
summary(mR)
...
LSmR<-lsmeans(mR, ~as.factor(clust))
contrast(LSmR, "pairwise")

Fig2a<- ggplot(data=data4, aes(x=as.factor(clust15), y=RaoQ, fill=as.factor(clust15), alpha=0.8))+
  geom_boxplot()+
  ggtitle("a)")+
  #theme(legend.position="none")+
...
mA<-lme(Avena ~treatment*as.factor(clust15), random=~1|year/shelterBlock, data4, na.action=na.exclude)
summary(mA)
...
Fig2b<- ggplot(data=data4, aes(x=as.factor(clust15), y=Avena, fill=as.factor(clust15), alpha=0.8))+
  geom_boxplot()+
  ggtitle("b)")+
  #theme(legend.position="none")+
...
ggplot(data4, aes(y=FDis, x=as.factor(clust15), fill=as.factor(clust15)))+
  geom_boxplot()

ggplot(data4, aes(y=H, x=as.factor(clust15), fill=as.factor(clust)))+
  geom_boxplot()

ggplot(data4, aes(y=ANPPgm, x=as.factor(clust15),  color=as.factor(clust15)))+
  geom_boxplot()+
  geom_smooth(method="lm", se=F)

data4 <- data4 %>% mutate(treatment=ordered(treatment, levels = c(controlRain="controlRain",  fallDry="fallDry", springDry="springDry", consistentDry="consistentDry")))
ggplot(data4, aes(y=forbCover, x=as.factor(clust15),  fill=treatment))+
  geom_boxplot()+
...
#data4$clust15 <- clust_xc$clust[match(data2$TB, clust_xc$TB)]
data4_cv<-data4 %>% group_by(treatment, clust15, shelterBlock) %>% summarize(CVanpp=sd(ANPPgm)/mean(ANPPgm)*100) #%>% 
data4_cv<- na.omit(data4_cv) %>% group_by(clust15, treatment)%>%summarize(CV=mean(CVanpp), se=sd(CVanpp)/sqrt(length(CVanpp)))
...
c<-lme(ANPPgm ~treatment2*as.factor(year), random=~1|shelterBlock, subset(data4, clust15==1), na.action=na.exclude)
summary(c)
anova(c) #treatment is significant
r.squaredGLMM(c) #12% of variation explained by fixed effects, 57% by whole model (interannual variation?)
qqnorm(residuals(c))
...
d<-lme(ANPPgm ~treatment2*as.factor(year), random=~1|shelterBlock, subset(data4, clust15=="2"), na.action=na.exclude)
summary(d)
...
ggplot(data4, aes(y=nbsp, x=as.factor(clust15)))+
  geom_boxplot()+
...
r.squaredGLMM(m4a) #30% of variation explained by fixed effects, 39% by whole model (interannual variation?)
...
ggplot(data = gf_graphic3, aes(x=year, y=meancover, shape=func)) + 
  ...
  geom_errorbar(aes(ymax = meancover+secover, ymin = meancover-secover), width=.25) + 
  labs(x="Year", y="Percent cover", fill="Functional group") +
  facet_wrap(~clust15*treatment2, ncol=4)+ 
  theme_bw()+ 
  theme(text = element_text(size=14))+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
