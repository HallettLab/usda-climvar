...
soil_2015<- soil_2015 %>% dplyr::select(-year, -X)
...
data2 <- filter(data, year =="2015", subplot!="C", subplot!="XC") %>% 
...
colSums(cover.Bio != 0) #note there are 16 species that occur 1 or 0 times, remove those
names(cover.Bio)<-str_replace_all(names(cover.Bio), c(" " = "_"))
cover.Bio2<- cover.Bio %>% dplyr::select(-Triteleia_hyacintha, -Trifolium_wildenovii,-Sonchus_oleraceus,-Sherardia_arvensis,
                                         -Senecio_vulgaris, -Rumex_pulcher, -Medicago_arabica, -Lupinus_bicolor, -Juncus_bufonius,
...
data2$H <- diversity(cover.Bio)
#calculate pielou's J
data2$J <- May_2015$H/log(specnumber(cover.Bio))

# run PCA
myrda2 <- rda(cover.Bio, scale = TRUE)
...
tr2 <- as.matrix(trait.dat2[,c(14:21)])
...
ggplot(tog, aes(x=PC1, y=PC2))+ 
  ...
  geom_point(aes(shape = subplot, color = treatment), size = 3) +
...
##Let's do this again but just for Both treatment
data2_B <- filter(data, year =="2015", subplot=="B") %>% arrange(treatment, shelterBlock)

Treatment<-data2_B[,4]
Year<-data2_B[,3]
data2_B$ID <- as.character(seq.int(nrow(data2_B)))
plotnames<-data2_B[,64]
cover.Bio_B<- data2_B %>% dplyr::select(-c(1:6), -ID)
rownames(cover.Bio_B)<-as.character(plotnames)
#check for empty rows
cover.Biodrop<-cover.Bio_B[rowSums(cover.Bio_B[, (1:57)]) ==0, ] #no empty rows, next step not needed
# remove empty rows cover.Biodrop<-cover.Bio[rowSums(cover.Bio[, (1:58)])  >0, ]
...
cover.rowsums <- rowSums(cover.Bio_B [1:57])

cover.relrow <- data.frame(cover.Bio_B /cover.rowsums)
cover.colmax<-sapply(cover.Bio_B ,max)
cover.relcolmax <- data.frame(sweep(cover.Bio_B ,2,cover.colmax,'/'))
cover.pa <- cover.Bio_B %>% mutate_each(funs(ifelse(.>0,1,0)), 1:57)
#calculate shannon-weiner
data2_B$H <- diversity(cover.Bio)
#calculate pielou's J
data2_B$J <- May_2015$H/log(specnumber(cover.Bio))

# run PCA
myrda3 <- rda(trB, scale = TRUE)

# extract values
siteout <- as.data.frame(scores(myrda3, choices=c(1,2), display=c("sites")))
siteout$ID<-rownames(siteout)
siteout$name <- siteout$ID

enviroout<-as.data.frame(scores(myrda3, choices=c(1,2), display=c("species")))
enviroout$type<-"species"
enviroout$name<-rownames(enviroout)

env<- soil_2015 %>% filter(subplot=="B")%>% dplyr::select(6:11) 
envvec<-envfit(myrda3,cover.Bio_B, na.rm=TRUE)
envvec
plot(myrda3)
plot(envvec, p.max=0.05) #add vectors to previous ordination
trait.dat2_B<- filter(trait.dat2, subplot=="B")
trB <- as.matrix(trait.dat2_B[,c(14:21)])
row.names(trB) <- trait.dat2_B$ID
envvec2<-envfit(myrda3,trB, na.rm=TRUE)
envvec2
plot(envvec2, p.max=0.05, col="orange")
envvec3<-envfit(myrda3,env, na.rm=TRUE)
envvec3
plot(envvec3, p.max=0.10, col="green")
...
env<- soil_2015 %>% dplyr::select(9:21) 
envvec<-envfit(trrda,cover.Bio, na.rm=TRUE)
envvec
...
colsT2 <- rep(c("Red","Blue","Orange", "Pink"), each = 12) #color based on drought treatment
cols1 <- rep(c("Red","Blue","Orange", "Pink"))
shapes <- rep(c(15, 8, 21 ), each=1) #shapes on subplot
shapes1 <- rep(c(15,8,21), each = 1)
points(siteout$PC1,siteout$PC2,col=colsT2,pch=shapes) 
text(trrda, display = "species", cex=0.9, col="grey40") #label species
plot(envvec3, p.max=0.10, col="green3", cex=0.9)
plot(envvec, p.max=0.02, cex=0.9)
# add legend for treatment
legend("topleft",legend=levels(data2_B$treatment), col=cols1, pch=19, cex=0.9,inset=0.1,bty="n",y.intersp=0.5,x.intersp=0.8,pt.cex=1.1)
# add legend for year
legend("bottomleft",legend=levels(sp), col="black", pch=shapes1, cex=0.9,inset=0.1,bty="n",y.intersp=0.5,x.intersp=0.8,pt.cex=1.1)
help(ordiplot)
...
# run PCA
tr_all<-read.csv("~/Desktop/above_traits.csv") %>% dplyr::select(-ANPPgm)
tr_1<-as.matrix(tr_all[,c(9:16)]) 
myrda.t <- rda(tr_1, scale = TRUE)

# extract values
siteout <- as.data.frame(scores(myrda.t, choices=c(1,2), display=c("sites")))
siteout$ID<-rownames(siteout)
siteout$name <- siteout$ID

enviroout<-as.data.frame(scores(myrda.t, choices=c(1,2), display=c("species")))
enviroout$type<-"species"
enviroout$name<-rownames(enviroout)

env<- soil_2015 %>% dplyr::select(6:11)
envvec<-envfit(myrda.t,cover.Bio2, na.rm=TRUE)
envvec
plot(myrda.t, display = "species")
plot(envvec, p.max=0.05) #add vectors to previous ordination
envvec3<-envfit(myrda.t,env, na.rm=TRUE)
envvec3
plot(envvec3, p.max=0.10, col="green")

#plots colored based on treatment
plot <- ordiplot(myrda.t,choices=c(1,2), type = "none")   #Set up the plot
colsT2 <- rep(c("brown3","forestgreen","dodgerblue3"), each = 1) #color based on drought treatment
cols1 <- rep(c("brown3","forestgreen","dodgerblue3"))
shapes <- rep(c(15, 8, 17 ), each=1) #shapes on subplot
shapes1 <- rep(c(15,8,17), each = 1)
points(siteout$PC1,siteout$PC2,col=colsT2,pch=shapes, cex=2) 
text(myrda.t, display = "species", cex=0.9, col="black") #label species
#plot(envvec3, p.max=0.10, col="green3", cex=0.9)
plot(envvec, p.max=0.01, cex=0.6, col="gray30")
...
ggplot(tog, aes(x=PC1, y=PC2))+ 
  geom_hline(aes(yintercept=0), color="grey") + 
  geom_vline(aes(xintercept=0), color="grey") +
  geom_point(aes(color = subplot), size = 3) +
  # scale_color_manual(values = c("grey20", "grey70")) +
  #geom_segment(data = envvec,
  #aes(x = 0, xend =  PC1,
  #y = 0, yend =  PC2),
  ...
  theme_bw() +theme(panel.grid.major=element_blank(),panel.grid.minor=element_blank(),
                    text=element_text(size = 15))+ 
  xlab(paste("Axis 1 (",sprintf("%.1f",myrda.t$CA$eig["PC1"]/myrda.t$tot.chi*100,3),"%)",sep="")) +
  ylab(paste("Axis 2 (",sprintf("%.1f",myrda.t$CA$eig["PC2"]/myrda.t$tot.chi*100,3),"%)",sep=""))
