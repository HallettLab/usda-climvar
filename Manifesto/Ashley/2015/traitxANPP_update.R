...
trait.dat1<- read.csv("Traits/Traits_GHscreening_Lina_Ashley/Combined_GHtraits.csv")
...
abovetr15_fd<-trait.dat1 %>% filter(Ht > 0) %>% dplyr::select(-ID, -GF, -Origin, -Trt) 
...
row.names(abovetr15_fd3)<-abovetr15_fd3$Taxon 
abovetr15_fd3 <- abovetr15_fd3 %>% dplyr::select(-Taxon)


aboveFD_2<-dbFD (abovetr15_fd3, cover15_fd3, w.abun = T, stand.x = F,
...
aboveFD_3<-dbFD (abovetr15_fd3, cover15_fd3, w.abun = T, stand.x = F,
...
#RaoQ of height
abovetr15_fd_ht<-abovetr15_fd3 %>% dplyr::select(Ht)
...
#RaoQ of SLA
abovetr15_fd_sla<-abovetr15_fd3 %>% dplyr::select(SLA)
...
#RaoQ of LDMC
abovetr15_fd_ldmc<-abovetr15_fd3 %>% dplyr::select(LDMC)
...
ggplot(aboveFD_2, aes(x=RaoQ, y=ANPPgm, group=subplot, color=subplot))+
  stat_smooth_func(geom="text",method="lm",hjust=0,parse=TRUE) +
  geom_point()+
...
rao_anpp<-ggplot(aboveFD.z, aes(x=RaoQ, y=ANPPgm, group=subplot, color=subplot))+
  ...
  #ylim(0,100)
  theme_bw()+
  geom_smooth(method='lm', se=FALSE, aes(group=subplot))
rao_anpp
